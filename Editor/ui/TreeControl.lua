...
local TreeControl = class("TreeControl",function(s,name,handle) return ccui.dragPanel({size = s,colorType = ccui.LayoutBackGroundColorType.solid, color = cc.c3b(255,255,255), opacity = 220}) end)
...
function TreeControl:createItem(node)
	...
	SET_POS(widget,pix(20)*node.D+pix(5),pix(10))
	node.W = widget
end
...
function TreeControl:Layout()
	...
	function calcH(node,h)
		...
		SET_POS(node.W,node.D*pix(20)+pix(5),h)
		--cclog("Layout :"..node.value)
		return h + pix(40)
	end

	local h = calcH(self._nodes[self._data[1]],pix(10))
...
function TreeControl:setIcon(icon,name)
	local node = self._nodes[name]
	if node then
		node.W:getChild("root"):setVisible(icon == TreeIcon.root)
		node.W:getChild("chr"):setVisible(icon == TreeIcon.chr)
		node.W:getChild("img"):setVisible(icon == TreeIcon.img)
		node.W:getChild("lua"):setVisible(icon == TreeIcon.lua)
		node.W:getChild("ui"):setVisible(icon == TreeIcon.ui)
		node.W:getChild("lua"):setVisible(icon == TreeIcon.root)
	end
end

function TreeControl:setCustomIcon(file,name)
	local node = self._nodes[name]
	if node then
		node.W:getChild("root"):setVisible(true)
		node.W:getChild("root"):loadTexture(file)
		node.W:getChild("chr"):setVisible(false)
		node.W:getChild("img"):setVisible(false)
		node.W:getChild("lua"):setVisible(false)
		node.W:getChild("ui"):setVisible(false)
		node.W:getChild("lua"):setVisible(false)
	end
end
