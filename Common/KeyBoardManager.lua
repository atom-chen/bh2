...
function KeyBoardManager:PressedKey(keycode,event)
	...
	if cc.KeyCode["KEY_L_ALT"] == keycode or cc.KeyCode["KEY_R_ALT"] == keycode then
		self._type = bor(self._type,alt_type)
	end

	if cc.KeyCode["KEY_L_CTRL"] == keycode or cc.KeyCode["KEY_R_SHIFT"] == keycode  then
		self._type = bor(self._type,ctrl_type)
	end

	if cc.KeyCode["KEY_L_SHIFT"] == keycode or cc.KeyCode["KEY_R_SHIFT"] == keycode  then
		self._type = bor(self._type,shift_type)
	end

	--cclog("press type:"..keylog[self._type])
	if self._using then
...
function KeyBoardManager:ReleasedKey(keycode,event)
	...
	--cclog("release type:"..keylog[self._type])
	if self._using then
		local shortcut = self._using
